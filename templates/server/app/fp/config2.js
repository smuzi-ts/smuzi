...
const Flow = (result) => ({
    ...
    map: function(nextTask) {
        ...
        console.log('this.result.value', this.result.value)
...
const fetchCalendars = (credentials) => {
...
const app = getCredentialsById(1)
    .then(result => fetchCalendars(result.value))
    .then(result => insertCalendar(result.value))
    .catch(error => error)
